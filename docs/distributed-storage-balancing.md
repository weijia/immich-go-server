...
| **Asset（资产）** | 一张照片/一段视频，由内部 `assetId = sha256(内容)`（**内容寻址**）唯一标识；物理文件名为 `<原始文件名片段>[_<assetId>][.<ext>]`（原始名在前便于按原名浏览，sha256 作为唯一后缀保身份/校验/去重），`<ext>` 取自摄入/上传时的原扩展名（小写，便于 OS 识别类型），`<原始文件名片段>` 为原始文件名经清洗/截断后的片段（对同名/超长/含非法字符做了安全处理以不超文件系统路径限制）。`assetId` 与物理文件名对客户端不透明；客户端侧 `deviceAssetId`（UUID 风格）经 `device_asset` 表映射到内部 `assetId` |
